...
local function dap_setup_lldb()
  ...
  reload.reload_module('./user/terminal2')
...
local config = {
  ...
  mappings = {
    ...
    t = {
      ...
      ["<C-q>"]     = { term.shutdown_active, desc = "Close Current Terminal" },
      ["<C-t>"]     = { term.new_term,        desc = "Spawn Additional Terminal(s)" },
    ...
    n = {
      ...
      ["tq"]       = { term.shutdown_all,     desc = "Shutdown All Terminals" },
      ["t<Right>"] = { term.bind_toggle('v'), desc = "Toggle Vertical Term" },
      ["t<Down>"]  = { term.bind_toggle('h'), desc = "Toggle Horizonal Term" },
